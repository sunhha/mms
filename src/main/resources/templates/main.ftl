...
<div class="menu">
    <ul class="yiji">
        <li style="background-color: #6f9ecf"><a href="${rc.contextPath}/main">物资管理</a></li>
        <li><a href="${rc.contextPath}/optRecord">出入库记录</a></li>
    </ul>
</div>
<div class="content">
    ...
    <div id="toolbar" style="float:right;">
        <button type="button" id="addButton" class="btn btn-primary" onclick="toAdd()">添加</button>
    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                名称:&nbsp;&nbsp;<input type="text" id="itemName"/>
                </br>
                </br>
                数量:&nbsp;&nbsp;<input type="number" id="itemNum"/>
                </br>
                </br>
                描述:&nbsp;&nbsp;<textarea id="itemDescription" style="height: 80px;"></textarea>
...
<!-- 模态框（Modal） -->
<div class="modal fade" id="ruku" tabindex="-1" role="dialog" aria-labelledby="rukuModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="rukuModalLabel">入库</h4>
            </div>
            <div class="modal-body">
                <input type="hidden" id="rukuId">
                数量:&nbsp;&nbsp;<input type="number" id="rukuNum"/>
                </br>
                </br>
                备注:&nbsp;&nbsp;<textarea id="rukuRemark"></textarea>

            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                <button type="button" class="btn btn-primary" onclick="ruku()">提交</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal -->
</div>
...
<!-- 模态框（Modal） -->
<div class="modal fade" id="chuku" tabindex="-1" role="dialog" aria-labelledby="chukuModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="chukuModalLabel">出库</h4>
            </div>
            <div class="modal-body">
                <input type="hidden" id="chukuId">
                数量:&nbsp;&nbsp;<input type="number" id="chukuNum"/>
                </br>
                </br>
                备注:&nbsp;&nbsp;<textarea id="chukuRemark"></textarea>

            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                <button type="button" class="btn btn-primary" onclick="chuku()">提交</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal -->
</div>
    ...
    var TableInit = function () {
        ...
        oTableInit.Init = function () {
            $('#dataTable').bootstrapTable({
                url: '${rc.contextPath}/item/data',         //请求后台的URL（*）
                method: 'GET',                      //请求方式（*）
                ...
                columns: [
                    {
                        ...
                        formatter: function (value, row, index) {
                            var html = "";
                            var rukuHtml = "<a href = '#' onclick = 'toRuku(" + JSON.stringify(row) + ")' >入库</a>&nbsp;&nbsp;";
                            var chukuHtml = "<a href = '#' onclick = 'toChuku(" + JSON.stringify(row) + ")' >出库</a>&nbsp;&nbsp;";
                            var delHtml = "<a href = '#' onclick = 'del(" + JSON.stringify(row) + ")' >删除</a>&nbsp;&nbsp;";
                            html = rukuHtml + chukuHtml + delHtml;
                            return html;
                        }
                    }
